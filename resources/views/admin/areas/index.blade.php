<x-layouts.app>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            <a href="{{ route('admin.dashboard') }}" class="text-gray-400 hover:text-tumbas transition-colors">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <div class="flex justify-between items-center mb-6">
                    ...
                    <button @click="showModal = true; editMode = false; currentArea = {}" 
                            class="bg-tumbas text-white px-4 py-2 rounded text-sm font-semibold">
                        + Tambah Area
                    </button>
                </div>
                            ...
                            @forelse ($areas as $area)
                                <tr class="hover:bg-gray-50 border-b">
                                    ...
                                    <td class="p-3 flex gap-2 justify-center">
                                        <button @click="currentArea = {{ $area }}; editMode = true; showModal = true" 
                                        ...
                                        <form action="{{ route('admin.areas.toggle', $area) }}" method="POST">
                                            @csrf
                                            <button class="px-3 py-1 rounded text-xs text-white {{ $area->is_active ? 'bg-stone-500 hover:bg-stone-600' : 'bg-green-600 hover:bg-green-700' }}">
                        ...
                        <div class="p-6">
                            ...
                            <form :action="editMode ? `/admin/areas/${currentArea.id}` : `{{ route('admin.areas.store') }}`" method="POST">
                                ...
                                <div class="mt-6 flex justify-end gap-3">
                                    <button type="button" @click="showModal = false" class="bg-gray-300 px-4 py-2 rounded text-sm">
                                        Batal
                                    </button>
                                    <button type="submit" class="bg-tumbas text-white px-4 py-2 rounded text-sm" x-text="editMode ? 'Simpan Perubahan' : 'Tambah'">
                                    </button>
                                </div>
                            </form>
                        </div>
